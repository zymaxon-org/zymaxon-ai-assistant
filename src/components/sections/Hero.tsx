...
const Hero = () => {
  ...
  return <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 -left-1/4 w-[800px] h-[800px] rounded-full opacity-40 blur-3xl animate-float" style={{
        background: 'radial-gradient(circle, hsl(var(--primary) / 0.25) 0%, transparent 60%)'
      }} />
        <div className="absolute bottom-1/4 -right-1/4 w-[700px] h-[700px] rounded-full opacity-30 blur-3xl animate-float-delayed" style={{
        background: 'radial-gradient(circle, hsl(var(--primary) / 0.3) 0%, transparent 60%)'
      }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] rounded-full opacity-15 blur-3xl animate-breathe" style={{
        background: 'radial-gradient(circle, hsl(var(--primary) / 0.4) 0%, transparent 50%)'
      }} />
        {/* Secondary accent blobs */}
        <div className="absolute top-[10%] right-[20%] w-[300px] h-[300px] rounded-full opacity-20 blur-2xl animate-float" style={{
        background: 'radial-gradient(circle, hsl(227 100% 70% / 0.4) 0%, transparent 70%)',
        animationDelay: '2s'
      }} />
        <div className="absolute bottom-[20%] left-[15%] w-[250px] h-[250px] rounded-full opacity-25 blur-2xl animate-float-delayed" style={{
        background: 'radial-gradient(circle, hsl(227 80% 65% / 0.3) 0%, transparent 70%)',
        animationDelay: '3s'
      }} />
      </div>
      ...
      <div className="absolute inset-0 opacity-[0.04] animate-grid-pulse" style={{
      backgroundImage: `
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {mounted && [...Array(35)].map((_, i) => <div key={i} className={`absolute rounded-full animate-particle ${i % 3 === 0 ? 'w-1.5 h-1.5 bg-primary/50' : 'w-1 h-1 bg-primary/30'}`} style={{
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {mounted && [...Array(6)].map((_, i) => <div key={`shape-${i}`} className="absolute w-16 h-16 border border-primary/10 rounded-lg animate-float opacity-30" style={{
        left: `${15 + i * 15}%`,
        top: `${20 + i % 3 * 25}%`,
          ...
          <div className={`transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="relative group cursor-pointer">
              ...
              <div className="relative w-28 h-28 md:w-36 md:h-36 lg:w-44 lg:h-44 rounded-full overflow-hidden shadow-card-hover transition-transform duration-500 group-hover:scale-110">
                <img src={zymaxonSymbol} alt="Zymaxon" className="w-full h-full object-cover" />
                {/* Shimmer effect */}
